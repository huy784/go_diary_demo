...
| 方法 | 路径 | 描述 |
|------|------|------|
| GET | /admin1/api/v1/diaries | 获取 admin1 的日记列表 |
| GET | /admin1/api/v1/diaries/:id | 获取 admin1 的单条日记 |
| POST | /admin1/api/v1/diaries | 创建 admin1 的日记 |
| PUT | /admin1/api/v1/diaries/:id | 更新 admin1 的日记 |
| DELETE | /admin1/api/v1/diaries/:id | 删除 admin1 的日记 |
| GET | /admin2/api/v1/diaries | 获取 admin2 的日记列表 |
| GET | /admin2/api/v1/diaries/:id | 获取 admin2 的单条日记 |
| POST | /admin2/api/v1/diaries | 创建 admin2 的日记 |
| PUT | /admin2/api/v1/diaries/:id | 更新 admin2 的日记 |
| DELETE | /admin2/api/v1/diaries/:id | 删除 admin2 的日记 |
...
2. 删除数据库容器：`docker container rm go-diary-postgres`
3. 删除数据卷：`docker volume rm go-diary_postgres_data`
4. 重新启动：`docker-compose up -d`
5. 启动应用：`go run src/main.go`
...
3. **数据隔离**：用户只能访问自己的日记
4. **中间件**：添加用户身份提取和验证
5. **API 路由**：调整为 `/admin1/api/v1/diaries` 格式
